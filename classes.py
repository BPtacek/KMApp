...
class Kingdom:
    ...
    def increase_level(self):
        ...
        else: print("Kingdom level cannot exceed 20!")
        
    def reduce_level(self):
        if self.level > 1:
            self.level -= 1
            self.update_control_DC()
        else: print("Kingdom level cannot be below 1!")
                
    def increase_skill(self,skill):
        if self.skills[skill.lower()] < 4: self.skills[skill.lower()] += 1
        else: print("Cannot raise skill proficiency above 4!")
        
    def reduce_skill(self,skill):
        if self.skills[skill.lower()] < 1: print("Cannot reduce skill below 0!")
        else: self.skills[skill.lower()] -= 1
        
    def get_skill(self,skill):
        if skill.lower() in self.skills: return self.skills[skill.lower()]
        else: print("Skill not in skill list!")
    ...
    def increase_attribute(self,attribute):
        ...
        else: print("Either attribute was mis-spelled or you tried to reduce it below 10. Don't do those things.")
        
    def reduce_attribute(self,attribute):
        if attribute.lower() in self.attributes and self.attributes[attribute.lower()] > 10:
            if self.attributes[attribute.lower()] > 18: self.attributes[attribute.lower()] -= 1
            else: self.attributes[attribute.lower()] -= 2
        else: print("Cannot reduce attribute below 10")
        
    def get_attribute(self,attribute):
        if attribute.lower() in self.attributes: return self.attributes[attribute.lower()]
        else: print("The input attribute was not in the attribute list, you should fix that.")
        
    def change_ruin(self,ruin,change):
        if ruin.lower() in self.ruins:
            if self.ruins[ruin.lower()] + change < 0: self.ruins[ruin.lower()] = 0
            else: self.ruins[ruin.lower()] += change
            
    def change_unrest(self,change):
        if self.unrest + change < 0: self.unrest = 0
        else: self.unrest += change
        
    def add_hex(self, coordinates):
        if coordinates not in self.claimed_hexes:
            self.claimed_hexes.append(coordinates)
            self.update_control_DC()
        else: print("You tried to add a hex that already belongs to the kingdom. That was pretty dumb.")
    ...
    def remove_hex(self, coordinates): # coordinates is a tuple (x,y) - a set of hex grid coordinates
        if coordinates in self.claimed_hexes:
            ...
            self.update_control_DC()
        else: print("You tried to remove a hex that didn't belong to the kingdom to begin with.")
        
    def get_size(self):
        num_hexes = len(self.claimed_hexes)
        if num_hexes == 0: return 0
        else: return max([j for (i,j) in size_thresholds.items() if i < num_hexes])
    ...
    def get_modifier(self,skill): 
        ...
        leadership_bonus_size = max({leadership_status_bonuses[i] for i in leadership_status_bonuses.keys() if i < self.level})
...
class Settlement:
    ...
    def destroy_building(self,building):
        if building in self.buildings.keys():
            ...
            else: self.buildings[Rubble] = 1 * building.lots
        else: print("You tried to delete a building that wasn't in the settlement's kingdom list.")
    ...
    def is_overcrowded(self):
        ...
        residences = {i:j for (i,j) in self.buildings.items() if i.residential} # types & numbers of residential buildings in settlement

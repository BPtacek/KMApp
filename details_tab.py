...
def draw_attribute_overview(state):
    ...
    target_frame = state.tabs["overview"]
...
def draw_attribute_details(state):
    ...
    target_frame = state.tabs["details"]
    ...
    attribute_table_frame.grid(row=5, column=0, rowspan=100)
...
def draw_name_table(state):
    kingdom = state.kingdom
    kingdom_details = state.tabs["details"]
    name_table_frame = tk.Frame(kingdom_details, borderwidth=1,relief="groove")
    name_table_frame.grid(row=5, column=2,sticky="nw")
...
def draw_level_table(state):
    ...
    level_table_frame.grid(row=6, column=2,sticky="nw")
    ...
    xp_label.grid(row=0, column=0,columnspan=3,sticky="w")
    increase_xp.grid(row=1, column=0)
    xp_entry.grid(row=1, column=1,sticky="w")
    xp_frame.grid(row=2,column=0,padx=5)
...
def draw_resource_table(state):
    ...
    resource_table_frame.grid(row=7, column=2,sticky="nw")
    ...
    resource_value = tk.Label(resource_frame,text="Stored")
    resource_value.grid(row=0,column=1,padx=2,columnspan=3)
    capacity_label = tk.Label(resource_frame,text="Capacity")
    capacity_label.grid(row=0,column=4,padx=2)
    gain_label = tk.Label(resource_frame,text="Gain/Turn")
    gain_label.grid(row=0,column=5,padx=2)
    def resource_capacity(resource,state):
        size = len(state.kingdom.claimed_hexes)
        category = max([i for i in size_thresholds.keys() if  i <= size])
        storage_buildings = {"ore":"Foundry",
                             "stone":"Stonemason",
                             "lumber":"Lumberyard",
                             "luxuries":"Secure Warehouse",
                             "food":"Granary"}
        num_stores = 0
        building_type = storage_buildings[resource]
        for settlement in state.kingdom.settlements:
            if building_type in settlement.readable_buildings().keys():
                num_stores += settlement.readable_buildings()[building_type]
        return str(size_thresholds[category]["storage"])
    def change_resource_amount(resource,state,stringvar,number):
        capacity = int(resource_capacity(resource,state))
        new_value = min(capacity, kingdom.resources[resource][0] + number)
        kingdom.resources[resource][0] = max(0,new_value)
        stringvar.set(value=str(kingdom.resources[resource][0]))
        state.write_headline_stats()
        state.update_stringvars()
    resource_startrow=1
    for resource in kingdom.resources.keys():
        ...
        stored = tk.StringVar(resource_frame, value=str(kingdom.resources[resource][0]))
        current = tk.Label(resource_frame, textvariable=stored)
        current.grid(row=resource_startrow, column=1)
        up_button = tk.Button(resource_frame, text="^",
                              command = lambda r=resource,st=state,sv=stored: change_resource_amount(r,st,sv,1))
        down_button = tk.Button(resource_frame, text="v",
                                command = lambda r=resource,st=state,sv=stored: change_resource_amount(r,st,sv,-1))
        up_button.grid(row=resource_startrow, column=2)
        down_button.grid(row=resource_startrow, column=3)
        capacity = tk.Label(resource_frame, text = resource_capacity(resource,state))
        capacity.grid(row=resource_startrow,column=4)
        try:
        ...
        gain.grid(row=resource_startrow,column=5)
        resource_startrow += 1
...
def draw_unrest_table(state):
    ...
    unrest_frame.grid(row=8,column=2,sticky="nw")
    ...
    unrest_penalty = tk.Label(main_frame, text="Penalty: " + str(-1 * kingdom.get_unrest_penalty()))
    unrest_label.grid(row=0,column=0)
    unrest_penalty.grid(row=0,column=1)
    change_unrest = tk.Label(main_frame,text="Change unrest by:")
    change_unrest.grid(row=1,column=0)
    unrest_change = tk.StringVar(main_frame,value="0")
    unrest_entry = tk.Entry(main_frame,width=6,textvariable=unrest_change)
    unrest_entry.grid(row=1,column=1)
...
def draw_ruins_table(state):
    ...
    ruins_frame.grid(row=9,column=2,sticky="nw")
    ...
    threshold_label.grid(row=0,column=2,columnspan=3)
    penalty_label.grid(row=0,column=5)
    startrow=1
    def update_threshold(ruin, state, stringvar, change):
        state.kingdom.change_ruin_threshold(ruin,change)
        stringvar.set(value=kingdom.ruins[ruin][1])
        state.write_headline_stats()
        state.update_stringvars()
    for ruin in Ruins:
        name = tk.Label(main_frame,text=ruin.title())
        name.grid(row=startrow,column=0)
        value = tk.StringVar(main_frame,value=kingdom.ruins[ruin][0])
        entry = tk.Entry(main_frame,width=2,textvariable=value)
        entry.grid(row=startrow,column=1)
        # value = tk.Label(main_frame,text=str(kingdom.ruins[ruin][0]))
        # value.grid(row=startrow,column=1)
        threshold_value = tk.StringVar(main_frame,value=kingdom.ruins[ruin][1])
        threshold = tk.Label(main_frame,textvariable=threshold_value)
        threshold.grid(row=startrow,column=2)
        v = threshold_value
        up_button = tk.Button(main_frame, text="^",
                              command = lambda r=ruin,s=state,v=v: update_threshold(r,s,v,1))
        down_button = tk.Button(main_frame, text="v",
                                command = lambda r=ruin,s=state,v=v: update_threshold(r,s,v,-1))
        up_button.grid(row=startrow, column=3)
        down_button.grid(row=startrow, column=4)
        penalty = tk.Label(main_frame,text="0")
        penalty.grid(row=startrow,column=5)                     
        def ruin_listener(event=None,state=state,entry=entry,value=value,ruin=ruin): # both this and kingdom.increase(xp) need lots of input sanitization!
            new_value = int(entry.get())
            state.kingdom.change_ruin(ruin,new_value)
            value.set(value=kingdom.ruins[ruin][0])
            state.write_headline_stats()
            state.update_stringvars()
        entry.bind("<Return>",lambda event,s=state,e=entry,v=value,r=ruin: ruin_listener(event,s,e,v,r))
        startrow += 1
